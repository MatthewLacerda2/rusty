use glam::{Vec3, Mat4};
use crate::core::scene::{Scene, ColliderComponent, ColliderShape};
...
pub fn test_narrowphase_collision(
    col_a: &ColliderComponent, world_mat_a: &Mat4,
    col_b: &ColliderComponent, world_mat_b: &Mat4,
    ...
    match (&col_a.shape, &col_b.shape) {
        ...
        (ColliderShape::Box { size: size_a }, ColliderShape::Box { size: size_b }) => {
            ...
            if overlap_x > 0.0 && overlap_y > 0.0 && overlap_z > 0.0 {
                if overlap_x < overlap_y && overlap_x < overlap_z {
                    ...
                    Some(CollisionContact { normal, depth: overlap_x })
                } else if overlap_y < overlap_x && overlap_y < overlap_z {
                    let normal = Vec3::new(0.0, if d.y > 0.0 { 1.0 } else { -1.0 }, 0.0);
                    Some(CollisionContact { normal, depth: overlap_y })
                } else {
                    let normal = Vec3::new(0.0, 0.0, if d.z > 0.0 { 1.0 } else { -1.0 });
                    Some(CollisionContact { normal, depth: overlap_z })
                }
            } else {
                None
            }
        ...
        (ColliderShape::Sphere { radius: r_a }, ColliderShape::Sphere { radius: r_b }) => {
            ...
            if len < sum_r {
                ...
                Some(CollisionContact { normal, depth: sum_r - len })
            } else {
                None
            }
        }
        ...
        (ColliderShape::Sphere { radius: r_s }, ColliderShape::Box { size: size_b }) => {
            ...
            if len < rad_s {
                if len > 0.0001 {
                    Some(CollisionContact { normal: dist_v / len, depth: rad_s - len })
                } else {
                    ...
                    if overlap_x < overlap_y && overlap_x < overlap_z {
                        let normal = Vec3::new(if d.x > 0.0 { 1.0 } else { -1.0 }, 0.0, 0.0);
                        Some(CollisionContact { normal, depth: overlap_x + rad_s })
                    } else if overlap_y < overlap_x && overlap_y < overlap_z {
                        let normal = Vec3::new(0.0, if d.y > 0.0 { 1.0 } else { -1.0 }, 0.0);
                        Some(CollisionContact { normal, depth: overlap_y + rad_s })
                    } else {
                        let normal = Vec3::new(0.0, 0.0, if d.z > 0.0 { 1.0 } else { -1.0 });
                        Some(CollisionContact { normal, depth: overlap_z + rad_s })
                    }
                }
            } else {
                None
        ...
        (ColliderShape::Cylinder { radius: r_a, height: h_a }, ColliderShape::Cylinder { radius: r_b, height: h_b }) => {
            ...
            if max_y_a <= min_y_b || min_y_a >= max_y_b { return None; }
            ...
            if depth_xz < depth_y {
                let normal = if len_xz > 0.0001 { d_xz / len_xz } else { Vec3::X };
                Some(CollisionContact { normal, depth: depth_xz })
            } else {
                Some(CollisionContact { normal: n_y, depth: depth_y })
            }
        }
        
        // 5. Cylinder vs Box
        (ColliderShape::Cylinder { radius: r_c, height: h_c }, ColliderShape::Box { size: size_b }) => {
            ...
            if max_y_c <= min_y_b || min_y_c >= max_y_b { return None; }
            
            let depth_xz = if len_xz > 0.0001 { rad_c - len_xz } else { rad_c };
            let (depth_y, n_y) = if pos_a.y > pos_b.y {
            ...
            if depth_xz < depth_y {
                let normal = if len_xz > 0.0001 { Vec3::new(dx, 0.0, dz).normalize() } else { Vec3::X };
                Some(CollisionContact { normal, depth: depth_xz })
            } else {
                Some(CollisionContact { normal: n_y, depth: depth_y })
            }
        ...
        // Box vs Cylinder
        (ColliderShape::Box { size: size_a }, ColliderShape::Cylinder { radius: r_c, height: h_c }) => {
            ...
            if max_y_a <= min_y_c || min_y_a >= max_y_c { return None; }
        ...
        (ColliderShape::Sphere { radius: r_s }, ColliderShape::Cylinder { radius: r_c, height: h_c }) => {
            let rad_s = *r_s * scale_a.max_element();
            let rad_c = *r_c * scale_b.x;
            let half_h_c = *h_c * scale_b.y * 0.5;
            ...
            let dir_xz = if len_xz > 0.0001 { d_xz / len_xz } else { Vec3::X };
            ...
            if len < rad_s {
                ...
                Some(CollisionContact { normal, depth: rad_s - len })
            } else {
                None
            }
        }
        ...
        // Cylinder vs Sphere
        (ColliderShape::Cylinder { radius: r_c, height: h_c }, ColliderShape::Sphere { radius: r_s }) => {
            let rad_s = *r_s * scale_b.max_element();
            let rad_c = *r_c * scale_a.x;
            let half_h_c = *h_c * scale_a.y * 0.5;
            ...
            for j in (i + 1)..num_entities {
                ...
                let contact = {
                    ...
                    test_narrowphase_collision(
                        ent_a.collider.as_ref().unwrap(), &world_matrix_a,
                        ent_b.collider.as_ref().unwrap(), &world_matrix_b,
                    )
                };
