...
impl PhysicsWorld {
    ...
    /// rejected by `accept` *during* parry's traversal — so the ray passes
    /// through excluded colliders and reports the nearest accepted hit instead of
    /// stopping (and missing) on an excluded one. This is the primitive the engine
    /// hitscan and the Lua bindings share via [`crate::physics::is_hittable`].
