...
fn a_second_instance_reflects_the_change_after_reimport() {
    ...
    a.get_entity_mut(root_a).unwrap().transform.position.y = 9.0;
    ...
    assert_eq!(b.get_entity(root_b).unwrap().transform.position.y, 9.0);
...
fn apply_a_child_field_resolves_the_root() {
    ...
    scene.get_entity_mut(child).unwrap().name = "Renamed".into();
    record_instance_overrides(&mut scene, root).unwrap();
...
fn applied_change_round_trips_through_save_load() {
    ...
    let (mut scene, root) = linked(&path);
    scene.get_entity_mut(root).unwrap().transform.position.x = 3.0;
    record_instance_overrides(&mut scene, root).unwrap();
    ...
    assert_eq!(scene2.get_entity(root).unwrap().transform.position.x, 3.0);
