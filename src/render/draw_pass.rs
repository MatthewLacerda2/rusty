...
use super::draw_resources::{
    AabbResource, AxisResource, GridResource, OutlineResource, Overlays, SolidResource,
};
use super::Renderer;
...
impl Renderer {
    ...
    fn run_shadow_passes(&mut self, encoder: &mut wgpu::CommandEncoder, scene: &Scene) {
        ...
        if !self.shadow_renderer.is_static_cached {
            self.shadow_renderer
                .render_static(&self.device, encoder, scene, &self.gpu_meshes);
        }
        self.shadow_renderer
            .render_dynamic(&self.device, encoder, scene, &self.gpu_meshes);
    }

    /// Rebuild the group-0 global bind group with the active skybox for reflections.
