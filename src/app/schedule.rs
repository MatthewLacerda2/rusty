...
//!
//! Allowed deps: app::stage, app::system.
...
/// `register` calls, then driven each tick by the `GameWorld`.
#[derive(Default)]
pub struct Schedule {
...
impl Schedule {
    ...
    pub fn run_startup(&self, world: &mut GameWorld, dt: f32) {
        self.run_stage(Stage::Startup, world, dt);
    }

    /// Run the per-frame stages in canonical order, each system in registration
    /// order. This is the real per-frame tick.
    pub fn run_frame(&self, world: &mut GameWorld, dt: f32) {
        for stage in Stage::FRAME_ORDER {
            self.run_stage(stage, world, dt);
        }
    }

    fn run_stage(&self, stage: Stage, world: &mut GameWorld, dt: f32) {
